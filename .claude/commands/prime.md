...
#   LMP Financial     → e.g. "The Enchanted Collective"
#   lmpfinancial.com            → e.g. "enchantedmadison.com"
#   Licensed independent mortgage broker (multi-LO shop)     → e.g. "luxury glamping and romantic experience property"
#   Lowell, Massachusetts (licensed in MA, NH, ME, RI, CT, FL, CO, VT, TX)          → e.g. "Madison, Indiana"
#   TBD post-Tuesday demo (2026-04-28)     → e.g. "June 2026"
#   Three-headed monster: borrowers in 9 licensed states, real estate agent referral partners, loan-officer recruits  → e.g. "romantic couples aged 27–45 within 90 min drive"
#   Residential mortgage origination across 9 loan programs (Fixed, FHA, VA, Jumbo, ARM, USDA, First-Time Buyer, Reverse, Interest-Only) via 22 LOs shopping multiple wholesale lenders        → e.g. "glamping tents, cottage, day-use hot tub escapes, proposal packages"
#   Convert site visitors into pre-approval applications routed to the right LO; capture realtor referral partners; recruit new LOs          → e.g. "drive direct bookings and own the romantic getaway category in southern Indiana"
#   Calendly per-LO inline embeds + existing per-LO *.my1003app.com/register pre-approval flow (preserved)    → e.g. "Lodgify (launch) / OwnerRez (scale)"
#   FinancialService (primary) + LocalBusiness (office) + Person per LO (NMLS as identifier)       → e.g. "LodgingBusiness" or "LocalBusiness" or "ProfessionalService"

---

You are the orchestrator for the **LMP Financial** (lmpfinancial.com) website build.
...
| Variable | Source |
|----------|--------|
| LMP Financial | initial-business-data.md Section 1 |
| lmpfinancial.com | initial-business-data.md Section 1 |
| Licensed independent mortgage broker (multi-LO shop) | initial-business-data.md Section 1 — one-phrase description |
| Lowell, Massachusetts (licensed in MA, NH, ME, RI, CT, FL, CO, VT, TX) | initial-business-data.md Section 1 |
| TBD post-Tuesday demo (2026-04-28) | initial-business-data.md Section 7 |
| Three-headed monster: borrowers in 9 licensed states, real estate agent referral partners, loan-officer recruits | initial-business-data.md Section 3 — specific, not generic |
| Residential mortgage origination across 9 loan programs (Fixed, FHA, VA, Jumbo, ARM, USDA, First-Time Buyer, Reverse, Interest-Only) via 22 LOs shopping multiple wholesale lenders | initial-business-data.md Section 2 |
| Convert site visitors into pre-approval applications routed to the right LO; capture realtor referral partners; recruit new LOs | initial-business-data.md Section 5 |
| Calendly per-LO inline embeds + existing per-LO *.my1003app.com/register pre-approval flow (preserved) | initial-business-data.md Section 5 — or "TBD — confirm before Phase 4" |
| FinancialService (primary) + LocalBusiness (office) + Person per LO (NMLS as identifier) | infer from business type — LocalBusiness / ProfessionalService / LodgingBusiness |
...
**Project:** lmpfinancial.com — new website build
**Client:** LMP Financial | Lowell, Massachusetts (licensed in MA, NH, ME, RI, CT, FL, CO, VT, TX)
**Business Type:** Licensed independent mortgage broker (multi-LO shop)
...
C:Projectsmp financial\.claude\commands\prime.md
...
Pass in prompt: PROJECT_FOLDER = C:Projectsmp financial
...
```bash
# Site
NEXT_PUBLIC_SITE_URL=https://lmpfinancial.com
...
Pass in prompt: PROJECT_FOLDER = C:Projectsmp financial
Output file: C:Projectsmp financial\src\data\site.ts
```
...
19. Route structure — /service-areas/[city]:
    ...
    A. src/app/service-areas/page.tsx — index page
       ...
       - generateMetadata: title "Service Areas | LMP Financial"
    ...
    C. src/app/service-areas/[city]/page.tsx — individual city page (SERVER component)
       ...
       - generateMetadata({ params }): unique title + description per city
         title: "[Service] in [City], [State] | LMP Financial"
...
Spawn with Agent tool (subagent_type: "general-purpose", run_in_background: false)
Pass in prompt: PROJECT_FOLDER = C:Projectsmp financial
Output file: C:Projectsmp financial\pre-launch-audit.md
```
...
1. Read `C:Projectsmp financial/pre-launch-audit.md` Summary section.
...
**Setup**
1. `cd C:Projectsmp financial/<next-app-folder>` → `npm run dev` with `run_in_background: true`
...
4. Log ALL findings to `C:Projectsmp financial/pre-launch-audit.md` under a new `## §Ultrareview Findings` section. Format per `knowledge/patterns/ultrareview-as-pre-launch-gate.md`.
